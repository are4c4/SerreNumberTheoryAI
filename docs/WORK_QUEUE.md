...
- `S3.2-LegendreSymbol` #56 is C-owned, draft PR #98. Exact head `45bde2eff8e75e901282151760b0c5dfc41a869a` remains the frozen `STACK-READY` subset **for #64**. A later Theorem 5(iii) repair head `527533d4…` was green in CI #217, but subsequent source-shaped cleanup exposed only local commutativity-normalization failures in `LegendreTwo.lean` (CI #220/#223). Current observed head `90996cbe1c4e1f2a6c2a2c2916d53467df8c94e0` is the next repair and CI #225 is queued. The old downstream freeze remains valid and must not be replaced by an unverified live head. Independent Blueprint exposition/linkage and final latest-main integration are still required.
...
- `C2S2.1-RootLiftingExistence` #99 is B-owned, draft PR #103. Earlier head `bf91ce4a…` passed CI #218 with Proposition 5 + Blueprint work. Later heads introduced/refined polynomial reduction compatibility and currently fail only inside the B-owned `RootExistence.lean`: observed head `6f4fe68e9276a5658a7bb091172f7dff24c22f83` fails CI #224 at the coefficient-level `MvPolynomial.map` compatibility and mapped-evaluation rewrite. A routed the exact diagnostics to B. Final normal root integration remains separately coordinated behind #98 because #103 uses a temporary top-level import hook to avoid the C-owned `Formalization.lean` hotspot.
...
| P7 | `S3.2-LegendreSymbol` | 3.2 Legendre記号 / 定理5 | `CLAIMED` | PR #98 live repair `90996cbe…`; CI #225 queued; keep #64 on frozen `45bde2ef…` | `work/s3-2-legendre-symbol` | #56 / C |
...
| P14 | `C2S2.1-RootLiftingExistence` | §2.1 命題5 | `CLAIMED` | PR #103 current observed head `6f4fe68e…` CI #224 local Lean failure; B repairing map/eval compatibility; root integration still after #98 | `work/c2-s2-1-root-existence` | #99 / B |
...
1. **#98 / C** keeps the next active `Formalization.lean` root-integration slot while its local Theorem 5(iii) repair and Blueprint/final §3.2 work finish. The explicit #64 freeze remains isolated from the moving live branch.
2. **#103 / B** keeps its Proposition 5 work isolated while fixing `RootExistence.lean`; once isolated CI is green and #98 frees the shared root, B should resync latest main, move the import into the normal `Formalization.lean` aggregator, remove the temporary hook, and re-run full CI before merge.
...
#100 was claimed by B while #99 is occupied with isolated proof/CI and the root hotspot. A therefore source-checked the immediately following §2.2 quadratic corollaries and split them into #104/#105 rather than inventing unrelated work.
