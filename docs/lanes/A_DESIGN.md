...
- B: #78 `C1-Supp-GaussLemma` / PR #115; #96 `C2S1.3-QpField`; #100 `C2S2.1-PrimitiveHomogeneousZeros`; #102 `C2S2.2-HenselLifting`; #104 `C2S2.2-HenselQuadraticOdd`; #105 `C2S2.2-HenselQuadraticTwo`.
...
- Unclaimed: #108 `C2S3.1-UnitFiltration`; #112 `C2S3.2-PrincipalUnits`; #120 `C2S3.3-PadicSquares`.
...
- #89/D draft PR #116 stacks exactly on the frozen #72 head. Latest checked head `382a56d40d41e197b837eab34f7de871d147d8ba` passed CI #267 and currently changes only `PadicIntegerMetric.lean`, so the metric implementation can proceed without shared-root contention. A requested separate stable subsets for #102 completeness and #96 topology+density when those declarations are ready.
...
- #105/B completed dyadic quadratic-lifting preflight on canonical branch `work/c2-s2-2-hensel-quadratic-two`, based on main `c8b94633…`. The source uses the #102 main Hensel theorem at `n=3,k=1`, derivative `2 * Σ aᵢⱼxᵢ`, mod-8/mod-4 valuation condition, and a determinant-unit reduction mod 2. Proof waits an explicit #102 main-theorem gate plus #72 domain/divisibility/valuation/primitive subset; no #96 dependency is needed.
...
- #120 is the newly seeded §3.3 p-adic-square-class preflight. Odd `p` uses valuation parity plus the merged project Legendre residue-square criterion; `p=2` uses valuation parity plus unit `≡1 mod 8`. Core proof waits #112 + #96, and no implementation begins before those exact interfaces are DONE/STACK-READY.
...
Owned executable/near-executable work includes #64/#78/#72 and stackable #89. Owned preflight/waiting work includes #100/#102/#104/#105/#96. The pool therefore retains several useful paths without speculative work.
...
4. monitor #116 implementation from exact `c43d7f09…` and future separate topology/completeness freezes needed by #96/#102;
5. monitor claims of #108/#112/#120 and refill only when safe capacity thins again;
