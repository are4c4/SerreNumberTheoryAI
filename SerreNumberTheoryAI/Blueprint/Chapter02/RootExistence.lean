...
書籍では有限levelの共通零点集合そのものを逆系にして、有限逆極限の非空性を直接適用する。
Leanの主証明は同じコンパクト性原理を、`SerrePadicInt` 上の減少する閉近似集合として実現している。
この違いは対象定理を強いp進定理で置き換えるものではなく、上で有限levelの逆系をなすことも別lemmaとして明示している。
:::
